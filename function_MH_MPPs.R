...
###  2. The MH algorithm for the DMPP
...
## 3. The MH algorithm for the Robust DMPP 1
  MH_Robust_DMPP_1 = function(n00,weight0,X0,Y0,n0,dim,a0,b0,mu0,Lambda0,X,Y,n,post_mean,post_sd,n_hist_trials){  
  s_weight=matrix(0,nrow=(n00+1),ncol=n_hist_trials)  ### the logit(weights) to be stored 
  weights=matrix(0,nrow=(n00+1),ncol=n_hist_trials) 
  class=matrix(0,nrow=(n00+1),ncol=n_hist_trials) 
  yesno=matrix(0,nrow=(n00+1),ncol=1)                ### for acceptance rate  # weight[n00+1] to be removed later
  return_f03=list(n00)
  ML_SC = rep(NA,n00)
  posterior = rep(NA,n00)
  ...
  beta_DMPP[1]<-a$beta_weight   
  posterior[1] = ML_SC[1] + 
    sum(log(0.9*(class[1,]==1)*(exp(dlogitbeta(s_weight[1,],alpha_DMPP[1],beta_DMPP[1],log=TRUE)))+
              0.1*(class[1,]==0))) + 
    dmvnorm(log(c(alpha_DMPP[1],beta_DMPP[1])),mean=c(1,1),sigma=matrix(c(1,0.5,0.5,1),nrow=2),log=TRUE)
  ...
  for (i in 1:(n00)){  
    ...
    s_weight[(i+1),]<-rnorm(n=n_hist_trials,mean=digamma(alpha_DMPP[i+1])-digamma(beta_DMPP[i+1]),
                            sd=sqrt(trigamma(alpha_DMPP[i+1])+trigamma(beta_DMPP[i+1]))) 
    class[(i+1),]<-rbern(n_hist_trials,0.9)
    weights[(i+1),]<-expit(s_weight[(i+1),])*class[(i+1),]
    return_f03[[i+1]] <- f0(weight=weights[(i+1),],X0,Y0,n0,dim,a0,b0,mu0,Lambda0,X,Y,n)
    ML_SC[i+1] <- return_f03[[i+1]]$ML0_SC0
    posterior[(i+1)] = ML_SC[(i+1)] + 
      sum(log(0.9*(class[(i+1),]==1)*(exp(dlogitbeta(s_weight[(i+1),],alpha_DMPP[i+1],beta_DMPP[i+1],log=TRUE)))+
                0.1*(class[(i+1),]==0))) + 
      dmvnorm(log(c(alpha_DMPP[i+1],beta_DMPP[i+1])),mean=c(1,1),sigma=matrix(c(1,0.5,0.5,1),nrow=2),log=TRUE)
    
    
    lr = posterior[i+1]-posterior[i]-
      sum(log(0.9*(class[(i+1),]==1)*dnorm(s_weight[i+1,],mean=digamma(alpha_DMPP[i+1])-digamma(beta_DMPP[i+1]),
                                           sd=sqrt(trigamma(alpha_DMPP[i+1])+trigamma(beta_DMPP[i+1])))  +  0.1*(class[(i+1),]==0)))+
      sum(log(0.9*(class[(i),]==1)*dnorm(s_weight[i,],mean=digamma(alpha_DMPP[i])-digamma(beta_DMPP[i]),
                                         sd=sqrt(trigamma(alpha_DMPP[i])+trigamma(beta_DMPP[i])))  +   0.1*(class[i,]==0))) -
      dtgamma(sigmasq_weight[i+1],shape=1,scale=5,truncation=mu_weight[i+1]*(1-mu_weight[i+1]),log=TRUE)-
      msm::dtnorm(mu_weight[i+1],0.3,0.3,0,1,log=TRUE) +
      dtgamma(sigmasq_weight[i],shape=1,scale=5,truncation=mu_weight[i]*(1-mu_weight[i]),log=TRUE)+
      msm::dtnorm(mu_weight[i],0.3,0.3,0,1,log=TRUE)
  ...
  return(list(s_weight=weights[-(n00+1),],yesno=yesno,posterior=posterior,class=class,
              mu_weight=mu_weight,sigmasq_weight=sigmasq_weight))
}



###  4.  The MH algorithm for the Robust DMPP 2
  MH_Robust_DMPP_2 = function(n00,weight0,X0,Y0,n0,dim,a0,b0,mu0,Lambda0,X,Y,n,post_mean,post_sd,n_hist_trials){  
  s_weight=matrix(0,nrow=(n00+1),ncol=n_hist_trials)  ### the logit(weights) to be stored 
  weights=matrix(0,nrow=(n00+1),ncol=n_hist_trials) 
  class=matrix(0,nrow=(n00+1),ncol=n_hist_trials) 
  yesno=matrix(0,nrow=(n00+1),ncol=1)                ### for acceptance rate  # weight[n00+1] to be removed later
  ...
  beta_DMPP[1]<-a$beta_weight   
  posterior[1] = ML_SC[1] + 
    sum(log(0.9*prod((class[1,]==1)*(exp(dlogitbeta(s_weight[1,],alpha_DMPP[1],beta_DMPP[1],log=TRUE))))+
              0.1*prod((class[1,]==0))))  + 
    dmvnorm(log(c(alpha_DMPP[1],beta_DMPP[1])),mean=c(1,1),sigma=matrix(c(1,0.5,0.5,1),nrow=2),log=TRUE)
  ...
  for (i in 1:(n00)){  
    ...
    s_weight[(i+1),]<-rnorm(n=n_hist_trials,mean=digamma(alpha_DMPP[i+1])-digamma(beta_DMPP[i+1]),
                            sd=sqrt(trigamma(alpha_DMPP[i+1])+trigamma(beta_DMPP[i+1]))) 
    class[(i+1),]<-matrix(rbern(1,0.9),1,n_hist_trials)
    weights[(i+1),]<-expit(s_weight[(i+1),])*class[(i+1),]
    return_f03[[i+1]] <- f0(weight=weights[(i+1),],X0,Y0,n0,dim,a0,b0,mu0,Lambda0,X,Y,n)
    ML_SC[i+1] <- return_f03[[i+1]]$ML0_SC0
    posterior[(i+1)] = ML_SC[(i+1)] + 
      sum(log(0.9*prod((class[(i+1),]==1)*(exp(dlogitbeta(s_weight[(i+1),],alpha_DMPP[i+1],beta_DMPP[i+1],log=TRUE))))+
                0.1*prod((class[(i+1),]==0))))  + 
      dmvnorm(log(c(alpha_DMPP[i+1],beta_DMPP[i+1])),mean=c(1,1),sigma=matrix(c(1,0.5,0.5,1),nrow=2),log=TRUE)
    
    lr = posterior[i+1]-posterior[i]-
      sum(log(0.9*prod((class[(i+1),]==1)*dnorm(s_weight[i+1,],mean=digamma(alpha_DMPP[i+1])-digamma(beta_DMPP[i+1]),
                                                sd=sqrt(trigamma(alpha_DMPP[i+1])+trigamma(beta_DMPP[i+1]))))  +  0.1*prod(class[(i+1),]==0)))+
      sum(log(0.9*prod((class[(i),]==1)*dnorm(s_weight[i,],mean=digamma(alpha_DMPP[i])-digamma(beta_DMPP[i]),
                                              sd=sqrt(trigamma(alpha_DMPP[i])+trigamma(beta_DMPP[i]))))  +   0.1*prod(class[i,]==0))) -
      dtgamma(sigmasq_weight[i+1],shape=1,scale=5,truncation=mu_weight[i+1]*(1-mu_weight[i+1]),log=TRUE)-
      msm::dtnorm(mu_weight[i+1],0.3,0.3,0,1,log=TRUE) +
      dtgamma(sigmasq_weight[i],shape=1,scale=5,truncation=mu_weight[i]*(1-mu_weight[i]),log=TRUE)+
      msm::dtnorm(mu_weight[i],0.3,0.3,0,1,log=TRUE)
